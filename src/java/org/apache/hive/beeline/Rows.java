...
abstract class Rows implements Iterator {
    ...
    final Boolean[] primaryKeys;//每一个列是否是主键,true表示是主键
    final NumberFormat numberFormat;//Number类型的列如何转换
    ...
    Rows(BeeLine beeLine, ResultSet rs) throws SQLException {
        ...
        int count = rsMeta.getColumnCount();//列的数量
    ...
    boolean isPrimaryKey(int col) {
        ...
        try {
            ...
            if (table == null || table.length() == 0 ||
                ...
                return (primaryKeys[col] = new Boolean(false)).booleanValue();//表或者列存在,则肯定不是主键
            }

            ResultSet pks = beeLine.getDatabaseConnection().getDatabaseMetaData().getPrimaryKeys(
                    beeLine.getDatabaseConnection().getDatabaseMetaData().getConnection().getCatalog(), null, table);//获取该表的主键结果集

            try {
                while (pks.next()) {
                    if (column.equalsIgnoreCase(
                            pks.getString("COLUMN_NAME"))) {//获取返回结果集中的主键name,判断是否是此时的列name
                        return (primaryKeys[col] = new Boolean(true)).booleanValue();//说明是主键
                    }
                }
            } finally {
                pks.close();
    ...
    class Row {//表示一行数据
        final String[] values;//存储数据
        final boolean isMeta;//true表示这一行是元数据,即是title
        boolean deleted;
        boolean inserted;
        boolean updated;
        int[] sizes;//每一个字段的值的长度

        //存储title信息
        Row(int size) throws SQLException {
            isMeta = true;
            values = new String[size];//title内容
            sizes = new int[size];//每一个title占用多少个长度
        ...
        Row(int size, ResultSet rs) throws SQLException {
            ...
            for (int i = 0; i < size; i++) {//从rs中读取一行信息
